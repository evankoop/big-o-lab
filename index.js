...
// ANSWER:
function findTheFourth(numbers) {
  let fourth = numbers[3]
  return fourth
}
//Time complexity is Constant (fixed amount of data)
...
function squareNumbers(numbers) {
  ...
  for (let i = 0; i < numbers.length; i++) {
    for (let k = 0; k < numbers.length; k++) {
      let numbersSquared = numbers[i] * numbers[k]
      answersArr.push(numbersSquared)
    }
  }
  return answersArr
}
//Quadratic is better because we are squaring the values in the array.
...
// ANSWER: Both answers' amount of space will be determined by the size of the string.
